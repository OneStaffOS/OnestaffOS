...
import styles from './lateness.module.css';
...
function LatenessRulesManager() {
  ...
  return (
    <div className={styles.latenessPage}>
      <div className={styles.header}>
        <h1>Lateness & Penalty Rules</h1>
        <p>Configure lateness thresholds, grace periods and per-minute deductions. Only HR Managers can access this page.</p>
      </div>
      ...
      <div className={styles.createForm}>
        ...
        <div style={{ flex: '1 1 320px' }}>
          ...
          <textarea value={newRule.description} onChange={(e) => setNewRule({ ...newRule, description: e.target.value })} placeholder="Short description (optional)" />
        </div>
        <div style={{ width: 160 }}>
          <label>Grace (mins)</label>
          <input type="number" value={newRule.gracePeriodMinutes} onChange={(e) => setNewRule({ ...newRule, gracePeriodMinutes: e.target.value })} placeholder="e.g. 5" />
        </div>
        <div style={{ width: 200 }}>
          <label>Deduction / min</label>
          <input type="number" value={newRule.deductionForEachMinute} onChange={(e) => setNewRule({ ...newRule, deductionForEachMinute: e.target.value })} placeholder="e.g. 0.5" />
        </div>
        <div>
          <label style={{ opacity: 0 }}>create</label>
          <button onClick={createRule} className={styles.createButton} disabled={loading}>
            Create Rule
          </button>
        </div>
      </div>
      ...
      <div className={styles.tableWrap}>
        <table className={styles.latenessTable}>
          <thead>
            <tr>
              <th>Name</th>
              <th>Description</th>
              ...
              rules.map((r: any) => (
                <tr key={r._id}>
                  ...
                  <td>
                    {editingId === r._id ? (
                      <>
                        <button className={styles.actionBtn} onClick={() => saveEdit(r._id)} style={{ marginRight: 8 }}>Save</button>
                        <button className={styles.actionBtn} onClick={() => { setEditingId(null); setEditValues(null); }}>Cancel</button>
                      </>
                    ) : (
                      <>
                        <button className={styles.actionBtn} onClick={() => { setEditingId(r._id); setEditValues({ ...r, gracePeriodMinutes: r.gracePeriodMinutes != null ? String(r.gracePeriodMinutes) : '', deductionForEachMinute: r.deductionForEachMinute != null ? String(r.deductionForEachMinute) : '' }); }} style={{ marginRight: 8 }}>Edit</button>
                      </>
                    )}
                  </td>
                </tr>
