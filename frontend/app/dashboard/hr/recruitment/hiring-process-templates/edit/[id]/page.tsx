/**
 * Edit Hiring Process Template Page
 * Route: /dashboard/hr/recruitment/hiring-process-templates/edit/[id]
 * HR Manager can edit existing hiring process templates
 */
...
export default function EditHiringProcessTemplatePage() {
  ...
  const fetchTemplate = async () => {
    try {
      ...
      const response = await axios.get(`/recruitment/hiring-process-templates/${templateId}`);
      ...
      setStages(template.stages.sort((a: ProcessStage, b: ProcessStage) => a.order - b.order));
    } catch (error: any) {
      console.error('Failed to fetch template:', error);
      alert('Failed to load template: ' + (error.response?.data?.message || error.message));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axios.put(`/recruitment/hiring-process-templates/${templateId}`, payload);
      alert('Hiring process template updated successfully!');
      router.push('/dashboard/hr/recruitment/hiring-process-templates');
    } catch (error: any) {
  ...
  if (loadingTemplate) {
    return (
      <ProtectedRoute requiredRoles={[Role.HR_MANAGER]}>
        <DashboardLayout title="Edit Hiring Process Template" role="Human Resources">
      ...
      <DashboardLayout title="Edit Hiring Process Template" role="Human Resources">
        <div className={styles.section}>
          <div style={{ marginBottom: '2rem' }}>
            ...
            <h1 style={{ marginBottom: '0.5rem' }}>Edit Hiring Process Template</h1>
            <p style={{ color: '#666', fontSize: '0.95rem' }}>
              Update the hiring process template and its stages
            </p>
          </div>
